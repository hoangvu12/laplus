...
- [x] An owned OpenCode session registers the generic per-thread MCP endpoint
      before the first prompt can use it
- [x] Registration carries the scoped authentication material produced by the
      MCP platform and never exposes it in snapshots or logs
- [x] MCP session lifetime follows the conversation and is released during
      every owned-session cleanup path
- [x] An external OpenCode instance receives no automatic MCP registration or
      configuration mutation
- [x] Registration failure becomes an actionable session startup failure rather
      than an apparently working agent with missing tools
- [x] Socket tests use a fake MCP platform and scripted OpenCode peer to assert
      registration ordering, authentication handling and cleanup
- [x] The OpenCode driver consumes the generic MCP interface and does not embed
