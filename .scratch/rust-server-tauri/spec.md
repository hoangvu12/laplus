...
- **Hard fork.** lightcode copies from t3code once and diverges. The contracts
  package is a *blueprint*, not a mirror; shapes are reimplemented in Rust to fit
...
- **The framing, not the schemas, is the risk.** The payload schemas are readable
  ...
  is undocumented and comes from an explicitly *unstable* module of the Effect
...
auth is out of scope, but the *handshake shape* is not optional — the client will
...
errors, resource scoping. These are read for *behaviour*, not structure. The Rust
...
re-running these tells you *the CLI moved* directly, without standing up a server
