...
- [ ] Installing on a clean Windows machine produces a working application
      — installed and working on _this_ machine; "clean" needs one this agent
      cannot provision
...
upstream's MIT notice. Whether that is _enough_ — `THIRD_PARTY_NOTICES.md` and
...
| --------------------- | ------------ | --------------------------- |
| **Installer (NSIS)**  | **5.05 MB**  | 14.95 MB _under_ the range  |
...
What _is_ enforced, in `cargo test`: the licence check against the real
...
The previous comment left this open — whether a copyright line is _enough_. It
...
measurement weighs what the installer _adds_ to the install directory, because
