...
why the server looked correct from every angle. It _was_ correct. It was
...
And the client is _written for that refusal_. `subscribeDynamic` is given
...
- Unless the call is a resume. `crate::threads::Watch` reads `afterSequence` —
  only whether it is _there_, not its value — because a client that sends one has
...
the turn still arrives live and every assertion about the _order_ of a turn is
...
sibling covers the resume that must _not_ be refused.
...
2. `probe-boot.mjs` — the client subscribes to the _real_ thread id it is about
...
rather than an architectural one: the criterion said _inside the window_, and
