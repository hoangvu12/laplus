...
- **`--deleted` is subtracted.** `--cached` lists what the _index_ holds, which
...
- **Directories are inferred.** Git names files and never the folders holding
  ...
  makes. The consequence is that an _empty_ directory does not appear in the
...
response does not _name_ the limit anywhere: it carries `truncated: true` and
...
`readFile` and `writeFile` are the first methods with a rule about _where_ they
...
A write checks its _parent directory_ rather than the file, because a file that
...
- **A refused write could still create directories outside the project.**
  ...
  `nested` outside it and _then_ refused the call. Refusing a write that has
...
- **`ExternalLauncher*` errors carry no `message`.** Unlike
  ...
  carries it, all five launcher errors define `message` as an override _getter_
