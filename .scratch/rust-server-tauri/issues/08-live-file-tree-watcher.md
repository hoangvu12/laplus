...
- [~] "**without a manual refresh**" — **declared divergence**, see below. The
  ...
  the _mounted tree_ still redraws on the refresh button, on reopening the
...
_remounting_ the panel inside thirty seconds does not re-fetch, because
...
it is not a consolation prize: the server now _knows_, so every answer after a
...
Two things it deliberately does _not_ filter, and both are load-bearing:
...
- **A workspace with nothing held.** There is nothing to invalidate, so the
  _rest_ of a long build is free even before the filter gets a say.
...
first version of this got that backwards. It refused the _new_ workspace, which
...
projects being _used_, because `listEntries` is the UI opening a project or
...
_before_ the delete, because afterwards there is no row to read it out of, and a
...
on Linux registers a watch per _directory_, so a large repository there can meet
...
- **A nested project was silently never updated.** `deliver` routed each event
  path with `find_map` over the watched roots, so it stopped at the _first_ root
...
- **A refused delete could still report success.** `remove_project` defaulted the
  ...
  that no longer exists with no way to find out. It is now checked _before_ the
...
- **A genuinely large repository.** The claim that a recursive Windows watch
  ...
  test builds is a couple of hundred files. What _is_ driven is that a burst of
