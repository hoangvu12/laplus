...
lines are *tests* rather than code, and they are worth more than they look: they
...
that is not free. The client *replaces* its buffer from the snapshot the event
...
- **Close takes the terminal out of the registry first and reaps with the lock
  ...
  can reach *that* session once it cannot be found. Reaping is the slow half and
...
What is deliberately *not* serialised is a close racing an attach that carries a
...
- **`deleteHistory` is not carried at all.** What this server keeps of a terminal
  is *in* that terminal, so closing one deletes its history whichever way the
...
- **`terminal.restart` accepts a missing size instead of refusing it**, where the
  ...
  missing size means is emphatically *not* the default, though — see below.
...
- **Scrollback is still in memory only.** Navigating away and back keeps
  ...
  because this ticket also requires that closing the app *reap* every terminal —
...
- **`hasRunningSubprocess` is still always `false`.** A process running in a
  ...
  asserts against a real four-second command. What is missing is the *busy dot*
...
- **"A process still running while detached continues"** runs a command that
  waits three seconds *and then* prints its marker, and asserts the marker is not
...
- **"Closing a terminal reaps its child processes"** cannot be asserted from the
  ...
  to grow *twice* before closing (a child that never started would make the
...
- **"Closing a terminal reaps its shell"** asserts the `exited` event arrives
  *before* the `closed` one. That event is published by the reaper, after the
