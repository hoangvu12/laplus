...
- `src/http.rs` — the two plain HTTP answers the UI needs _before_ it will open
  a socket.
...
(`effect/unstable/rpc/RpcClient.ts`, `effect@4.0.0-beta.78`): it fails _every_
in-flight request and _every_ open subscription on that socket, and the
...
be the _normal_ answer to the UI's own boot sequence, and each one would tear
...
compares _keys and JSON types_, never _values_. Two divergences are therefore
...
would need an allow-list of fields that _legitimately_ differ per machine —
...
by inference from a _different_ ticket's screenshot is the same move this ticket
...
That id is the _draft_ — the "New thread" pane that was open. Six threads in the
