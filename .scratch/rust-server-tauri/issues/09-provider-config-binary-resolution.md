...
- [~] Tests cover the resolution order and each failure mode through the socket
  ...
  _answer_ is read through the socket and no test needs an agent installed, but
...
_replaces_ the last extension rather than adding one, so a command whose name
...
setting; it is a statement about a specific file, usually the install _directory_
...
asserts this against a `Search` that _does_ hold a working binary, so the test
...
_configuration_, which is this ticket's subject.
...
there is a _stored preference_ over these slugs, which is ticket 22's.
...
_nearest_ unreachable model — for 2.1.100 it says "too old for Claude Opus 4.7,
...
about it is test-shaped, and the test that drives it drives _it_, not a variant of
it.
...
- `getProviderSummary` tests `!provider.installed` **before** it looks at `status`,
  ...
  branch for an _absent_ instance.
...
_before_ it compares, so what is held against the capture is a **ready provider with
...
- **`socket_provider.rs`** (16 tests) drives only what the UI observes. Precedence
  is asserted through the reported _version_ rather than the resolved path — two
...
- **The `.cmd` fakes prove the npm-shim claim as a side effect.** The ticket says
  shim following is dead weight; a test binary that _is_ a launcher script and
