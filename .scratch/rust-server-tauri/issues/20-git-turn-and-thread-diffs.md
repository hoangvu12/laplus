...
untracked file, the binary file. The one that is a _decision_ is the eighth: a
...
Three real bugs, all in the same seam — what a checkpoint _claims_ — and each
...
- **A checkpoint's `status` is how the turn went, not whether the capture
  ...
  `ready` means `completed`. So a _failed_ turn was being quietly relabelled as
...
- **A turn the developer stopped gets no checkpoint at all.** The same rule with
  ...
  and `error`, so _none of them means interrupted_ and any row would undo what
