...
- [x] A prompt sent from the real UI reaches the agent and is acknowledged
      ...
      including the `bootstrap.createThread` without which the UI's _first_
...
a `streaming` flag, and the flag _is_ accumulate-and-reconcile: `true` appends,
...
including the client's one exception: an _empty_ buffered message leaves the
...
So `protocol::SessionState::fold_line` now _returns_ what the line changed
...
and a durable write records the number it _was given_ rather than choosing one.
...
- **The shell snapshot reports the counter, not the stored column.** They differ as
  soon as a conversation is under way, and a snapshot reporting the _stored_ number
...
the criterion asks for are shown — and they are the agent's _own_ account of both,
...
socket and `protocol` keeps for the CLI. It is deliberately a _ratio_: a turn that
...
not a program, which is the one unusable case that does _not_ fall back. The
...
11 owns continuity and persistence _together_ for exactly that reason. Storing
...
- **The turn read the thread before applying the composer's selection.** A model
  ...
  with the model the thread was _created_ with, so a mid-conversation model change
...
- **A unit test was running the developer's real `claude`, against the real API.**
  ...
  which is the one unusable case that does _not_ fall back to `PATH`, and the
...
- **The session could only ever become `running` once per process.** The
  ...
  is not `running` makes `bind_assistant_message` settle the turn at the _first_
  ...
  now counts _starts_ — the scripted agent logs one line per process — and asserts
...
- **The shared counter did not actually order the two aggregates.** Projects took
  ...
  event numbered 6 is not reordered by the client, it is _dropped_ — the project
...
queries used to _insert_ the slot they asked about, so any id a client mentioned
