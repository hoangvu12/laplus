...
- [x] An error reported by the agent appears in the conversation and the session
      ...
      in neither
- [x] A turn can be retried after an error — the retry is answered by the _same
      process_, which is what says the error ended the turn and not the session
...
What changed is a clause in the sentence: _"Turn completed in 2.0s · 2
unrecognised events and 1 unreadable line"_. Two decisions fell out of writing it:
...
- **`system`/`compact_boundary`.** It fell to `SystemEvent::Other`, which is
  silence. Compaction is a fact about what the _agent_ can still see; the
...
it argued the old cost, so the reversal is recorded where a reader of _that_
...
- **Drift between turns was reported by nobody.** The count was anchored to the
  ...
  subtracted straight back out. The anchor moved from the turn to _the last
  report_, which also gave the death path something to say: a turn that never
...
_Standing_, which the change had introduced as a concept and not as a word.
...
_two_, and its own counter starts at zero, so the scripts are keyed to the
