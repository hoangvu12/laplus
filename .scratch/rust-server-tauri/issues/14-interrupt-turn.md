...
- [x] A new message can be sent immediately afterwards in the same conversation —
      including _while the stopped turn is still winding down_, which is the one
...
moves the latest turn to `interrupted` on it _immediately_
...
the next turn _before_ the CLI has finished aborting the last one — and the
...
it, with a script that pauses _after_ being written to so the window is a second
...
puts the turn _back_ — because a turn marked stopped that nobody managed to stop
...
set only _after_ the line has reached the child.
...
`control_response` is replayed with a stop _before_ it, where a `control_request`
...
delegated, not driven.** The child in question is the _tool's_ — a `Bash` running
