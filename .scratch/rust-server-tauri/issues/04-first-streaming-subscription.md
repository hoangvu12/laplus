...
- [x] The UI subscribes and receives an initial update — *proven against the
      harness client, not a browser; see comments*
...
- `src/subscriptions.rs` — the streaming mechanism, and the only new *idea* in
  ...
  without touching it.
- `src/config_store.rs` — the configuration as a *live* value rather than a
