...
- [x] Rejecting returns control to the agent and the session remains usable —
      ...
      rather than in error, and the _same_ child takes the next turn
...
agent _asking_, and nothing in `--help`, in the spike's write-up, or in the six
...
- **`08`, declined.** The tool result is the deny _message_, with
...
This was the load-bearing discovery on the _other_ protocol, and it made the
...
- **`payload.requestKind` is not optional in practice.** A request whose kind is
  none of `command`, `file-read`, `file-change` is _dropped from the panel
  entirely_ — silently, so the developer would see a stopped conversation with
  ...
  and a _file-read_ as a question.
...
- **Answering a request the session does not know is refused _in the words the
  client recognises_, carrying the id.** `derivePendingApprovals` also drops a
...
to the turn _in flight_. Putting both on one channel would have parked the
...
The agent is written to _before_ the resolution is published, for the matching
...
assert on the line the _agent_ was written rather than only on the row the client
...
- **"For the session" is the CLI's own suggestion handed back**, not a rule this
  ...
  request that offered nothing is approved _once_, and the row says "Approved"
...
not to that function: the CLI's own default _is_ to ask, and what was missing was
...
number it holds, so the row that _did_ happen could be dropped as stale.
...
- **The escape hatch did not work — neither half of it.** The failure row was
  ...
  carries **no `requestId`** — and `derivePendingApprovals` requires the id _and_
  ...
  error _before_ anything was published at all, so there was no row of any kind.
...
- **The subprocess-leak assertion was vacuous.** It read `live_agents()` on the
  _restarted_ server, where the count is zero because nothing ever ran there.
...
- **A permissioned tool call renders as two rows, not one.** The CLI announces
  the `tool_use` block _before_ it asks about it, so the work log reads
...
- **A request the CLI abandons while the session lives on stays open.** The
  driver settles outstanding requests when it _ends_; nothing settles one the
