...
- [x] Several tool calls within one turn are rendered in order and correctly
      paired — the CLI interleaves each call with its own result, *and* every row
...
- [x] Thinking is shown as distinct from assistant output — its own row, in the one
      kind this UI renders with its thinking affordance, and *out* of the reply,
...
- **A tool result arrives as a `user` message.** Not under a tool-specific event
  ...
  pass, and which is about echoing the *developer's* turns back. So the `user`
...
- **Calls and results interleave in block order** even when the model was asked for
  ...
  collapse of an invocation into its result is *adjacency*-based
  ...
  change to satisfy — and the UI is upstream's. It is *not*, on its own, enough:
...
So `Turn` now carries `text` *and* `content`: the text blocks joined, and every
...
the row appears when the reasoning block *closes*, so it is a record of a pause
...
timidity: `showDestructiveRowStyle` styles an error row as the *server's* failure.
...
*output*, because that is the new information and the row it collapses into already
...
- **"The order needs no sorting" was false, because the client sorts.**
  ...
  carry one and falls back to `createdAt` — a *millisecond* — and then to a rank that
...
- **"Counted as drift" was also false.** The ticket and `protocol.rs` both claimed an
  ...
  Before this ticket such a block was at least *visible*, as a `[?]` in the flattened
...
- **`status: "completed"` does not stop the client guessing.**
  `workEntryIndicatesToolFailure` short-circuits on `"failed"` and *not* on
...
- **Thinking rows inherited the same heuristic**, and that one *was* avoidable. A
...
- **The capture-coverage counters could not fail.** They were only inserted where the
  ...
  have made the key *vanish* from the totals — and a key that is absent cannot be
...
one is an absent *key* rather than a `null` — a `null` would fail the client's decode
...
with the id missing failed to parse the *whole line* — losing the reply beside it
...
the *API* runs, so their results come back as their own block types rather than as a
...
- **A message and an activity inside one millisecond may render out of order.**
  ...
  which is real time; the pair that *can* land together is a result and the next
