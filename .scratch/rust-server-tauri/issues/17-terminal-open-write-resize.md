...
- **An oversized `terminal.write` is refused rather than truncated.** The
  ...
  the queue in front of the shell is bounded in *slots*: sixty-four slots of
...
- **Scrollback.** 18 owns "scrollback from before detaching is still present
  ...
  pane under load. What is *not* here is 18's version of it — scrollback is in
...
on this wire delivers *replacements*, so a client seeing an update its snapshot
...
- **Resize** is asserted by asking the shell how big it thinks it is (`mode con`,
  ...
  *program* does with the size it is given, so asking the program is the only
  honest test that does not need an emulator.
- **Colour** is asserted as a control sequence arriving *as* a control sequence:
  ...
  what it re-renders, so byte-exactness from the *program* was never this
