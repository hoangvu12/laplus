...
- [~] A very long transcript loads without stalling the UI — a 400-message
  ...
  subscription's own task rather than the socket's read loop. What is _not_
...
- Persistence is rows in SQLite, and its only job is that a developer can _read_
...
governs the transcript governs the table — a write happens at a _message
boundary_, and a reply of any length costs one row.
...
with deltas that _disagree_ with the buffered message, because that is the only
...
_then_ flushes, in that order, because a session publishes its last changes on
...
- **The registry says which conversations there are, not the database.** A thread
  reaches the database _eventually_, so the stored rows are a subset of what
...
unit test, because a test cannot kill its own process. The _graceful_-close path
...
an append counter — a buffered message _replaces_ one the deltas already put in
...
on a _snapshot_ that shows the session settled, and this is not
...
- **The refusal's most useful sentence was read before it could exist.** The
  driver asked `Agent::complaint()` and _then_ reaped the child — but stdout and
  ...
  _returns_ the complaint, having joined the stderr reader — bounded by the same
...
- **`Threads::forget` detached the driver it was releasing.** It dropped the
  ...
  the _handle kept_, parked on `Inner::winding_down`, which shutdown drains with
...
- **The delete asked the database which conversations existed, and the database
  ...
  thread reaches that table _eventually_ — that is this ticket's whole design. A
...
and is now _found_ by id like the message beside it; and the long-transcript
...
- **A boot that is slow because the history is large.** `Shell::new` reads every
  ...
  was rejected because the project list opens with every thread's _summary_, so a
...
- **A conversation whose agent session the CLI _silently_ forks.** If a resumed
...
over them. Worth naming now: fourteen tickets remain and three of them (12, 13, 20) are the substantial ones.
