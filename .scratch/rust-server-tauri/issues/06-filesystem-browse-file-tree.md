...
back _after_ the two-thousand-file listing rather than before it.
...
termination. A filesystem that cycles can only make the listing _fill up_.
...
still succeeds. Only the root is different, because its contents _are_ the
answer.
...
What _is_ tested is the case a test can make anywhere: a dangling symlink, an
