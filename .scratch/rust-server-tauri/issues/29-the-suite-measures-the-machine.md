...
_next_ run. Several of the confusing failures above were self-inflicted that way.
...
paid by every _other_ ticket. Two have already paid it — 28 spent three runs and
...
it _gave up_ — not that it did so within N milliseconds. The two cases below are
...
full second and _then_ returned the give-up result would still pass on an idle
...
So change what `READ_TIMEOUT` _is_. Its only job is to stop a wedged test hanging
...
_before_ their afternoon disappears, not in a closed ticket. Put them in the
...
waited for the child _cannot_ return `TimedOut`, and `assert_eq!(…, TimedOut)`
