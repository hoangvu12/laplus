...
now keeps _whole_ stderr rather than its first line, because a blocked switch's
...
- **Switching to a remote ref makes the local branch that tracks it.** The
  picker can list `origin/feature`, and a working tree cannot be _on_ one, so a
...
- **`refKind: "remote"` answered with the branches nobody had checked out.**
  The fold — dropping `origin/main` when `main` is there — is about a _pair_ of
...
the filesystem's limit on one _part_ of a name, so applying it to the whole name
...
- **The listing is capped at 10,000 refs** (`MAX_REFS`), and unlike the status's
  file cap this one bounds the _answer_ as well as the work: `totalCount` counts
...
- **`Project::repository_identity` is still `null`.** Ticket 19's notes named
  ...
  to the _project registry_ rather than to any of these four calls. It stays
