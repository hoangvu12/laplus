...
- [x] DNS, TLS, wrong-environment, authentication, WebSocket, and Cloudflare Access interception failures are distinguishable; the last successful verification and stale state are retained. **The client half was missing until now.** Ten typed kinds crossed the wire and `apps/web/src` read none of them, so a developer saw one observation about a probe and never which of ten things to go and fix. `FAILURE_KIND_LABELS` is now an exhaustive `Record<ExternalTunnelFailureKind, string>` — an eleventh kind is a compile error — rendered in the external-endpoint step, which had no failure text at all, as well as the compact row and the connector status.
...
- [x] Only a verified endpoint is advertised as available; its HTTPS/WSS endpoint, external ownership, and layered health appear in Connections. **The WSS origin reached only one screen.** It was rendered inside the external-endpoint step and nowhere else, so a laplus-managed endpoint stated its HTTPS origin alone; the dedicated panel and the row now state both.
