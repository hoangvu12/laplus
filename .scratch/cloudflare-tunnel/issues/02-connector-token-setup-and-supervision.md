...
- [x] Running-server and UI-driver tests use a fake cloudflared process to prove restart, shutdown, readiness, persistence, verification, and secret-redaction boundaries. **Five of six; verification is the one a driver cannot reach.** The running-server half proves all six. The driver now proves the other five at 170 verdicts, up from 95: this ticket's own connector-token panel in a third world with no account certificate written at all, driven end to end with the stand-in asserting `--token` is absent from argv; supervision restart by `SIGKILL` three times, proving `restartCount` 1/2/3, a different pid each time, `degraded` in the window, a budget that survives recovery, `restart-exhausted` with no fourth launch, Start refused and Retry refilling; and persistence by stopping the server and starting a replacement on the same port and data directory, reading back every field checkbox 3 names — with "setup was not re-run" proven by the token file's mtime rather than by an empty field. Redaction is asserted across four boundaries including every file in the scratch tree. **Verification stays out and is not faked**: it needs a hostname that resolves in public DNS and an inbound HTTPS path to this machine, and is covered against the hermetic verifier in `http_public_exposure.rs`. Two facts found on the way: `restart-exhausted` answers `readiness: null` rather than `false`, and the driver was red at `3cf96ae` because that commit gave Forget a confirmation and the driver still pressed the old label — which had been silently skipping ticket 01's whole external-registration block.
