...
aarch64, driven by hand from small Python and Node harnesses rather than by any
laplus code.
...
| `07-provider-probe.jsonl`    | initialize, concurrent account/model/skills reads, and startup noise                 |
...
Machine identifiers are replaced because `.scratch/` is committed and this
repository is public: `installationId` is zeroed, `serverName` reads
`redacted-host`, paths use `<home>` and `<workspace>`, and the account email is
`developer@example.com`. They identify the machine or account rather than the
protocol, and nothing reads them. There are no credentials in these files —
every `…Token…` key in them is a token _count_ from
