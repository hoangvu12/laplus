...
// `laplus-server`'s own default, from `launch.rs`. Deliberately not upstream's
// 3773, so a reference server and laplus can run side by side.
const DEFAULT_SERVER_PORT = 4773;
...
function resolveDevProxyTarget(
  ...
  // An explicit websocket URL still wins over the default below — that is how
  // you point the UI at a server on another host or another port.
  if (wsUrl) {
    try {
  ...
  // Nothing configured: assume the server this UI is part of, on its own
  // default port. Upstream derived this from `scripts/dev-runner.ts`, which
  // started both halves and passed the port it had chosen; laplus has no such
  // runner — `pnpm dev:server` and `pnpm dev` are two commands — so the default
  // has to live here or `pnpm dev` serves a UI that proxies nowhere.
