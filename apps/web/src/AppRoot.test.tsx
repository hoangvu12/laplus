...
describe("AppRoot", () => {
  it("shares the application atom registry with routed UI and renderer-wide desktop hosts", () => {
    ...
    // Ticket 27. Outside the router because a window's controls belong to the
    // window, not to a route; it renders null anywhere but laplus's own shell.
    expect(isValidElement(children[3]) && children[3].type).toBe(DesktopWindowControls);
  });
});
