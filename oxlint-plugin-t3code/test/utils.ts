...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    // oxlint's own entry point rather than either shim in `.bin`. Both shims
    // are unspawnable on Windows: the extensionless one is a shell script and
    // fails with ENOENT, and `oxlint.CMD` fails with EINVAL because Node
    // refuses to spawn `.cmd` without a shell. The entry is an extensionless
    // JavaScript file with a shebang, so running it under `node` works on every
    // platform and needs no branch on which one this is.
    ...
    const output = yield* spawnAndCollectOutput(
      ChildProcess.make("node", [oxlintEntry, "--config", configPath, sourcePath], {
        cwd: repoRoot,
      }),
    );
