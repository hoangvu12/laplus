...
async fn a_conversation_and_its_transcript_survive_a_restart() {
    ...
    // The work log too: what the session was and what the turn cost are the two
    // things a developer scrolls back for.
    let kinds: Vec<&str> = snapshot["thread"]["activities"]
        ...
        .collect();
    assert!(kinds.contains(&"session.init"), "{kinds:?}");
...
async fn a_restored_conversation_is_continued_by_resuming_the_agents_own_session() {
...

    {
        ...
        // The session the agent announced, which is the id the next run has to
        // resume. Read off the activity because that is where a client sees it.
        assert_eq!(
            activity(&events, "session.init")["payload"]["activity"]["payload"]["sessionId"],
            SESSION
        );
        assert!(
            agent.resumed().is_empty(),
