...
//! lightcode has neither yet — so the *cause* cannot come over the socket.
...
use lightcode_server::config::{ClaudeSettings, ProviderState, ServerConfig};
...
async fn the_real_agent_on_this_machine_is_found_and_reports_its_version() {
    if std::env::var_os("LIGHTCODE_TEST_REAL_AGENT").is_none() {
        eprintln!(
            "skipped: set LIGHTCODE_TEST_REAL_AGENT=1 to resolve the Claude Code \
