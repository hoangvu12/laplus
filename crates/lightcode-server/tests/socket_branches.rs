...
//! `lightcode_server::refs`: a repository is made with the `git` binary and then
...
fn cloned() -> (Workspace, Workspace) {
    ...
    clone.git(&["config", "user.name", "lightcode tests"]);
    clone.git(&["config", "user.email", "tests@lightcode.invalid"]);
