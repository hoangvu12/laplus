...
//! Nothing here reaches into `lightcode_server::terminal`: a shell is started by
...
async fn a_terminal_opens_in_the_project_and_answers_what_is_typed_into_it() {
    ...
    pane.run(&mut client, &echo("lightcode-was-here")).await;
    pane.wait_for(&mut client, "lightcode-was-here").await;
...
async fn colour_reaches_the_client_as_a_control_sequence_rather_than_as_text() {
    ...
    pane.run(&mut client, &colour_command("lightcode")).await;
    pane.wait_for(&mut client, RED).await;

    // The colour is there, and the text it colours is there…
    assert!(pane.text().contains("lightcode"), "{}", pane.text());
