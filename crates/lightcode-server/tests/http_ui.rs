...
//! `lightcode_server::ui` and its route into `axum`, not the vendored assets,
...
const PAGE: &[u8] = b"<!doctype html><title>lightcode</title><div id=root></div>";
...
/// upgrade the webview makes is one [`lightcode_server::auth`] already accepts,
