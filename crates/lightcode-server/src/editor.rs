...
//! `EditorId` against that exact list of literals — an id lightcode invented
...
fn spawn(command: &str, args: &[String]) -> std::io::Result<()> {
    ...
    // exits, which is what "open my editor" means — closing lightcode must not
...
mod tests {
    ...
    fn the_file_manager_can_be_started() {
        if std::env::var_os("LIGHTCODE_TEST_LAUNCH_EDITOR").is_none() {
            eprintln!(
                "skipped: set LIGHTCODE_TEST_LAUNCH_EDITOR=1 to let this open a real window"
            );
            return;
        }
