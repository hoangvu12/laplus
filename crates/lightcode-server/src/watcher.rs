...
impl Watcher {
    ...
    pub fn watch(&self, key: &str, root: &Path) {
        ...
        if notify.is_none() {
            match self.start() {
                ...
                Err(error) => {
                    eprintln!("lightcode: cannot watch the filesystem: {error}");
                    return;
                }
            }
        }
        ...
        if let Err(error) = started.watch(root, RecursiveMode::Recursive) {
            eprintln!("lightcode: cannot watch {}: {error}", root.display());
            return;
        }
        ...
        if watched.len() >= MAX_WATCHED {
            ...
            eprintln!(
                "lightcode: {MAX_WATCHED} workspaces are already watched, so changes \
    ...
    fn start(&self) -> notify::Result<RecommendedWatcher> {
        ...
        notify::recommended_watcher(move |event| match event {
            ...
            Err(error) => eprintln!("lightcode: a filesystem event was lost: {error}"),
        })
    }
}
