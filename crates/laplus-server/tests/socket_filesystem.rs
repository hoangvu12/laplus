...
//! [`laplus_server::filesystem`] — a folder is picked by sending the request
...
async fn the_filesystem_can_be_browsed_to_pick_a_folder_for_a_new_project() {
    ...
    assert_eq!(names(&filtered), ["lightbulb", "lighthouse"]);
    assert_eq!(
        filtered["entries"][0]["fullPath"],
        json!(projects.join("lightbulb").to_string_lossy())
    );
