...
//! request 6).
//!
//! **laplus sends the two together, and that is a declared divergence** —
...
impl Repositories {
    ...
    fn remember(&self, root: &WorkspaceRoot) -> Arc<Repository> {
        ...
        if kept.len() >= MAX_WATCHED {
            ...
            eprintln!(
                "laplus: {MAX_WATCHED} working trees are already being kept, so the status \
        ...
        match read(repository.path()) {
            ...
            Err(why) => {
                ...
                if !state.reported {
                    state.reported = true;
                    eprintln!(
                        "laplus: cannot read the working tree status of {}: {}",
...
fn read(root: &Path) -> Result<Status, Unavailable> {
    ...
    if files.len() > MAX_FILES {
        eprintln!(
            "laplus: {} has {} changed files; the status names the first {MAX_FILES}",
            root.display(),
            files.len()
        );
...
mod tests {
    ...
    fn only_changes_that_could_move_a_status_are_acted_on() {
        ...
        for ignored in [
            ...
            ".git/refs/laplus/checkpoints/6162/turn/1",
        ] {
