...
| Installed on disk | **24.33 MB** | inside, 5.67 MB of headroom |
| Application binary | **24.25 MB** | inside, 5.75 MB of headroom |
...
The spec's signal to stop and re-evaluate is roughly 20,000 lines of Rust, and the figure it is about is **production code: 12,164**, 7,836 lines inside it. The total above it is mostly prose and unit tests — a third of this server by line is its own tests — and reporting *that* against the signal would write a false alarm into every build.
