...
| `cloudflare-tunnel.mjs`      | Dedicates an inactive Cloudflare tunnel through the Connections wizard and reads the ownership, the connector and the deletion verdict back **off the server**. Cloudflare ticket 05. **Starts its own server** against a stand-in `cloudflared` and certificate — see its header |
...
`cloudflare-tunnel.mjs` is the one script that **starts the server it drives**,
