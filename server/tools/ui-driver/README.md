...
`cloudflare-tunnel.mjs` is the one script that **starts the servers it drives**,
because the alternative is pointing a probe at somebody's real Cloudflare
account. It writes a stand-in `cloudflared` onto a scratch `PATH`, writes a
