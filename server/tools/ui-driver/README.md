...
| `cloudflare-tunnel.mjs`      | Dedicates an inactive Cloudflare tunnel and creates a new one through the Connections wizard, and reads the ownership, the connector and the deletion verdict back **off the server**. Cloudflare tickets 05 and 06. **Starts two of its own servers** against stand-in `cloudflared`s and certificates — see its header |
