...
| `cloudflare-tunnel.mjs`      | Dedicates an inactive Cloudflare tunnel, creates a new one, stops and restarts both, forgets the adopted one and deletes the created one everywhere — through the Connections wizard, reading ownership, the connector, the cleanup state and the resources actually removed back **off the server** and **off a stand-in Cloudflare DNS API**. Cloudflare tickets 05, 06 and 07. **Starts two of its own servers** against stand-in `cloudflared`s and certificates — see its header |
...
It does that **twice**, on `<port>` and `<port>+1`, because adoption and creation
each end with a connector laplus supervises and the wizard rightly will not offer
setup for an exposure that already exists. One browser walks both: the second
server is a different origin and is booted exactly the way the first was, with the
...
**What it cannot cover is verification and pairing.** Both need a hostname that
genuinely resolves and a public HTTPS path back to this machine, and the whole
point of the scratch world is that there is neither. They are covered against the
hermetic verifier in `tests/http_cloudflare_adoption.rs` and
`tests/http_cloudflare_creation.rs` instead.
