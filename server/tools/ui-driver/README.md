...
| `probe-usage.mjs`            | Drives the Usage sidebar/direct route at desktop and narrow widths, checks its range and total, and proves a raw transcript sentinel is absent from captured socket frames                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `probe-session-stop.mjs`     | Sends a message, then deletes the thread — which is one of the two places the client sends `thread.session.stop`, and the only one this server can reach. Reads the session back **off the server**. Thread-lifecycle ticket 04; wants a stand-in agent that holds a session                                                                                                                                                                                                                                                                                                                                        |
| `cloudflare-tunnel.mjs`      | Dedicates an inactive Cloudflare tunnel, creates a new one, runs a third from a connector token alone, stops and restarts them, kills a connector out from under its supervisor, restarts a whole server, forgets the adopted tunnel and deletes the created one everywhere — through the Connections wizard, reading ownership, the connector, the cleanup state and the resources actually removed back **off the server** and **off a stand-in Cloudflare DNS API**. Cloudflare tickets 02, 05, 06 and 07. **Starts three of its own servers** against stand-in `cloudflared`s and certificates — see its header |
| `probe-subagent.mjs`         | Spawns a real **background subagent** and watches two things at once: whether its row is ever visible, and whether the composer reads finished while it is still working. Takes the URL (with `#token=`) and a wait in seconds. Spends a real agent turn; its prompt is deliberately one that cannot change the checkout it runs in. Found the duplicate ending and the renamed row that the recorded fixture could not show                                                                                                                                                                                        |
| `probe-queued-prompt.mjs`    | Sends a second prompt **while the first turn is running** and samples what the composer offers, by `aria-label`, twice a second. Exit 1 if it ever says "Connecting" on a conversation that is working, or if the reply stops growing. Spends two real agent turns; both prompts are incapable of changing the checkout. What showed the queued-prompt bug in the window and, run against a pre-fix binary, that it was there                                                                                                                                                                                       |
| `probe-attach-image.mjs`     | Picks an image the way a phone has to — the composer only ever took a paste or a drop, and a phone offers neither. Expands the collapsed mobile composer, presses the paperclip and hands its file input a real PNG through `DOM.setFileInputFiles`, checking desktop width first so a regression there is not invisible from a phone. **Starts its own server** against isolated app data and a provider pointed at a text file; sends nothing and spends nothing                                                                                                                                                  |
...
| `probe-provider-toggle.mjs`  | Lists the Providers panel's enable switches and, given an index, presses one and prints the `server.updateSettings` it sent beside the answer. Every provider toggle in the window was refused for a reason only this made visible — a settings patch that names one control carries the whole panel's shape, and a refusal moves the switch back with nothing said                                                                                                                                                                                                                                                 |
...
reached. `probe-session-stop.mjs` sends one too and needs the opposite of that: a
stand-in that _holds_ a session, because a stop with no process behind it is the
no-op case rather than the one under test. Its header has the script. Everything
else here spends nothing.

Three scripts **start the servers they drive** rather than attaching to a
running one: `cloudflare-tunnel.mjs`, `probe-provider-lifecycle.mjs` and
`probe-attach-image.mjs`. `cloudflare-tunnel.mjs` was the first, because the
alternative is pointing a probe at somebody's real Cloudflare account. It writes a stand-in `cloudflared` onto a scratch `PATH`, writes a
stand-in account certificate beside it, starts an isolated `laplus-server` with
`--ui ../../../apps/web/dist`, and takes the boot credential out of that
server's own startup lines — so it needs no `sqlite3` and no second window, but
it does need the web bundle built first. `laplus-server` embeds no UI; only the
shell does, and the shell needs a display.

It does that **three times**, on `<port>`, `<port>+1` and `<port>+2`, because
adoption, creation and the connector-token path each end with a connector laplus
supervises and the wizard rightly will not offer setup for an exposure that
already exists. One browser walks all three: each later server is a different
origin and is booted exactly the way the first was, with the credential in the
URL fragment.

The third world is ticket 02's, and it is the one that writes **no account
certificate at all** — its whole claim is that a developer with an existing
`cloudflared` and one tunnel's connector token never signs in to anything, and a
world where a certificate merely went unused would not test that. It is also the
world where the driver stops being polite:

- **It kills the connector**, three times, which is `MAX_RESTARTS`. Stop and
  Start are the operator's buttons and reach `set_desired`; nothing a developer
  can press reaches `child_failed`, so `restartCount`, `degraded` and
  `restart-exhausted` are unobservable from the UI alone. The stand-in writes
  its pid on every launch, and a `cloudflared.slow` file makes each replacement
  slow to answer `/ready` — `degraded` is the window between the two, and a
  stand-in that binds instantly makes it too short to read.
- **It restarts the server**, which the other two worlds never do, and reads
  each field checkbox 3 names back **by name**. The token file's modification
  time is what proves setup was not re-run: `configure` is the only thing that
  writes it, which is a stronger answer than an empty field on a screen.

It also stands up a **stand-in Cloudflare DNS API** on loopback, pointed at with
`LAPLUS_CLOUDFLARE_API` — which the server honours only towards loopback,
because the request carries a Cloudflare API token in a header. That is not a
shortcut round the CLI but a different surface: `cloudflared` has no `route dns
delete`, so the record a deletion removes goes through Cloudflare's REST API, and
a stand-in that answered it as another `cloudflared` verb would let the driver
pass against a command that does not exist. The delete half asserts on that
API's own record list, so "the exact recorded DNS record and no other" is read
from the thing that was asked rather than from the screen that asked it.

**What it cannot cover is verification and pairing, and that is the whole of
what it cannot cover.** Both need a hostname that genuinely resolves in public
DNS and a public HTTPS path back to this machine, and the whole point of the
scratch world is that there is neither — faking either would make the driver
assert against itself. They are covered against the hermetic verifier in
`tests/http_cloudflare_adoption.rs`, `tests/http_cloudflare_creation.rs` and
`tests/http_public_exposure.rs` instead. Everything up to the probe is driven
here: that an unverified endpoint is never advertised, and that a locally ready
connector is not a publicly verified one.
