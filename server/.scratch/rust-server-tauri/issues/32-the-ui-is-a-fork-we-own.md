...
- **The spec's non-goals move.** "The UI is reused unmodified and is upstream's"
  stops being true, and user story 57 — the *unmodified* upstream UI connecting
...
- **`pnpm` is now load-bearing.** It always was, informally — `dist/` was built
  by hand — but it was building *someone else's* code. A stale `dist/` is now a
...
it. Worth recording here because *nothing in `packages/contracts` showed it* —
