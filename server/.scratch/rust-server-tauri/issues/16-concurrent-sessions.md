...
- **"Independent" within one project means server-side isolation only.**
  ...
  the *same folder at the same time* and may edit the same files; last write
  ...
  word is *independent*, not *conflict-free*, and they are different promises.
...
- **Two harness constraints, already checked.** `binaryPath` is one server-wide
  setting, so both sessions spawn the *same* `ScriptedAgent`, whose `starts()`,
...
- [x] Ending or interrupting one session does not disturb the others — the
      ...
      another turn
- [x] Two conversations within the same project remain independent — *server-side*,
...
- [~] Subprocesses are tracked per session so none is orphaned when one ends —
      ...
      the other conversation answering. The *tracking* half is proved; whether a
...
- `SocketClient::open_conversation` registered the folder as `project-1`.
  ...
  *folder* as well, because `project.create` refuses a root another project
  already holds.

### Proving "simultaneously" without asserting on a clock

The first criterion is the only one that is a claim about *time*, and the first
...
`binaryPath` is one server-wide setting, so both conversations spawn the *same*
...
interrupting, so an *undisturbed* conversation that reached that script would hang
...
Both were found by the spec review, and both were tests that were *true* but did
...
- **"Each agent runs in its own project's working directory."** The first version
  ...
  *other* project's folder would produce exactly the same two markers. Fixed by
...
- **"No subprocess is orphaned."** `Threads::forget` decrements `live_agents`
  ...
  *per session* — one child released, one not. Whether the released driver was
...
- **ADR-0002** records why `threads.rs` is not split into a conversation log and a
  ...
  splitting.
- **ADR-0003** records that *independent* and *conflict-free* are different
