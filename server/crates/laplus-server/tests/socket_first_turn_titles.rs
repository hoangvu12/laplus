...
async fn generate(State(generator): State<Generator>) -> Json<Value> {
    generator.prompts.fetch_add(1, Ordering::SeqCst);
    generator.started.notify_one();
    generator.release.notified().await;
    Json(json!({"parts":[{"type":"text","text":"{\"title\":\"Focused socket titles\"}"}]}))
}
...
async fn generator() -> (String, GeneratorControl) {
    ...
    (endpoint, GeneratorControl { prompts, started, release })
}
