...
/// The permission mode in effect is what the conversation is set to, and the
/// agent is started the way that mode means.
///
/// It used to assert a third thing — the mode the *agent itself* reported, off a
/// `session.init` row, which is what is actually in force once the CLI has
/// applied the developer's own settings file over what it was asked for. That
/// row is gone (see `crate::turn`, `Folded::Initialized`), and with it the only
/// place the two could be seen to differ. What is left is both ends of what this
/// server controls: what the conversation says, and what the process was given.
#[tokio::test]
async fn the_permission_mode_in_effect_is_visible() {
    ...
    // The agent was started the way that mode means. `approval-required` is
