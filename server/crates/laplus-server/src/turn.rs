...
pub(crate) async fn prompt_content(text: &str, attachments: &[crate::threads::PromptAttachment]) -> Result<Vec<serde_json::Value>, String> {
    ...
    for attachment in attachments {
        if !crate::attachments::SUPPORTED_MIMES.contains(&attachment.mime.as_str()) {
            return Err(format!("Unsupported Claude image attachment type '{}'.", attachment.mime));
        }
        let bytes = tokio::fs::read(&attachment.path).await.map_err(|error| format!("Failed to read Claude image attachment: {error}"))?;
        content.push(json!({"type":"image","source":{"type":"base64","media_type":attachment.mime,"data":crate::attachments::encode_base64(&bytes)}}));
    }
    Ok(content)
}
...
impl Driver for Claude {
    ...
    async fn send(&mut self, prompt: &crate::threads::Prompt) -> std::io::Result<()> {
        ...
        let content = prompt_content(&prompt.text, &prompt.attachments).await.map_err(std::io::Error::other)?;
