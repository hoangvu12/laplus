...
//!
//! ```text
//! refs/laplus/checkpoints/<thread>/turn/0   before the first turn
//! refs/laplus/checkpoints/<thread>/turn/1   after it, and before the second
//! refs/laplus/checkpoints/<thread>/turn/2   …
//!
...
//! this module knows about. `git for-each-ref refs/laplus` is the whole of
...
///
/// Under `refs/laplus/` rather than `refs/heads/` or `refs/tags/` so that
...
pub(crate) const REFS: &str = "refs/laplus/checkpoints";
...
fn cut_notice() -> String {
    format!(
        "\n\n*** This diff was truncated by laplus: it is larger than {} MB, and only the \
...
const AUTHOR: &str = "laplus";
const EMAIL: &str = "checkpoints@laplus.invalid";
...
///
/// `refs/laplus/checkpoints/…^{commit}` cannot be mistaken for a path, a tag
    ...
    fn new() -> ScratchIndex {
        ...
        ScratchIndex {
            path: std::env::temp_dir().join(format!(
                "laplus-checkpoint-{}-{unique}.index",
                std::process::id()
            )),
        }
    }
...
mod tests {
    ...
    fn a_checkpoint_ref_is_a_ref_name_git_allows() {
        ...
        assert!(named.starts_with("refs/laplus/checkpoints/"), "{named}");
    ...
    fn a_truncated_patch_stops_at_a_whole_file_and_says_so() {
        ...
        assert!(kept.contains("truncated by laplus"), "{kept}");
    }
