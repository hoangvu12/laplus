 ...
 * via a session cookie. A cross-origin `fetch` does not send cookies by default,
 * so those requests must opt into credentialed mode; bearer connections carry
 * their credential in a header and need no cookies. Applied per-request via
 ...
 * primary/local connections carry no credential, and bearer connections send a
 * static `Bearer` token.
 */
