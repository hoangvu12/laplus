...
export type PrepareThreadSessionInput = CommandInput<"thread.session.prepare">;
...
export type RejectThreadUserInputInput = CommandInput<"thread.user-input.reject">;
...
export const pinThread: (input: PinThreadInput) => CommandEffect = Effect.fn(
  ...
  return yield* dispatch({ ...input, type: "thread.pin", commandId: yield* commandId(input) });
});

export const unpinThread: (input: UnpinThreadInput) => CommandEffect = Effect.fn(
  "EnvironmentCommands.unpinThread",
)(function* (input) {
  return yield* dispatch({ ...input, type: "thread.unpin", commandId: yield* commandId(input) });
});
...
export const prepareThreadSession: (input: PrepareThreadSessionInput) => CommandEffect = Effect.fn(
  "EnvironmentCommands.prepareThreadSession",
)(function* (input) {
  const metadata = yield* timestampedCommandMetadata(input);
  return yield* dispatch({
    ...input,
...
export const rejectThreadUserInput: (input: RejectThreadUserInputInput) => CommandEffect =
  Effect.fn("EnvironmentCommands.rejectThreadUserInput")(function* (input) {
    const metadata = yield* timestampedCommandMetadata(input);
    return yield* dispatch({
      ...input,
